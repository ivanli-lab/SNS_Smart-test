...
async function checkWebsite2(url) {
    ...
    try {
        ...
        // 6️⃣ Handle Room Popups
        setCurrentStep2('termsAndConditions');
        console.log('[Step 6] Handling Room popups...');
        await newPage.waitForTimeout(5000);
        
        // 【關鍵】先關閉直播間頁面的翻譯列
        console.log('[Step 6] Dismissing translate bar on room page...');
        await dismissTranslateBar(newPage).catch(() => {});
        await newPage.waitForTimeout(2000);

        const vp = newPage.viewportSize() || { width: 1280, height: 720 };
        console.log(`[Step 6] Viewport: ${vp.width}x${vp.height}`);

        for (let i = 0; i < 25; i++) {
            const content = await newPage.evaluate(() => document.body.innerText).catch(() => '');
            console.log(`[Step 6] Scan ${i+1}: length=${content.length}`);
            
            if (content.includes('How to Play')) {
                console.log('[Step 6] Detected "How to Play"');
                await newPage.evaluate(() => {
                    const target = Array.from(document.querySelectorAll('label, div, span, p')).find(el => (el.innerText || '').includes("Don't show this again"));
                    if (target) {
                ...
                await smartClick(newPage, /START PLAYING|START/i, 'Start Playing');
                await newPage.waitForTimeout(2000);

            } else if (/CLAIM NOW/i.test(content) || /Freespin|Free Spin|SPIN/i.test(content)) {
                // Free Spin 彈窗：先用元素定位，失敗則用座標轟炸
                console.log('[Step 6] Detected Free Spin popup - attempting CLAIM NOW...');
                let claimed = await smartClick(newPage, /CLAIM NOW/i, 'CLAIM NOW (text)');
                if (!claimed) {
                    // 座標轟炸備案：根據screenshot廝算按鈕約在頁面中大50%　1新瑵8h65%處
                    const clickX = Math.round(vp.width * 0.5);
                    const clickY = Math.round(vp.height * 0.64);
                    console.log(`[Step 6] Coordinate fallback: clicking (${clickX}, ${clickY})`); 
                    await newPage.mouse.click(clickX, clickY, { delay: 150 });
                    claimed = true;
                }
                await newPage.waitForTimeout(2500);

            } else if (content.includes('Congratulations')) {
                console.log('[Step 6] Detected Congratulations popup');
                await smartClick(newPage, /CONTINUE|Confirm|確認|繼續/i, 'Room Congrats');
                await newPage.waitForTimeout(1500);

            } else {
                console.log('[Step 6] No blocking popup detected. Proceeding.');
                break;
            }
            
            // 驗證是否還有阻擋性彈窗 (移除SPIN的汎化匹配)
            const blocking = await newPage.evaluate(() =>
                Array.from(document.querySelectorAll('div, p, span, h1, h2')).some(el => {
                    const t = el.textContent || '';
                    const r = el.getBoundingClientRect();
                    return (t.includes('How to Play') || t.includes('Congratulations') || /CLAIM NOW/i.test(t)) 
                        && el.offsetParent !== null && r.width > 100 && r.height > 30;
                })
            ).catch(() => false);
            if (!blocking) { console.log('[Step 6] All popups cleared!'); break; }
            await newPage.waitForTimeout(1500);
        }
